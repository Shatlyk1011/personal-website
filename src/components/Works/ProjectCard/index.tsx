    ...
    <div className="flex gap-[3.2rem] border-b border-white/[12%] pt-[6.4rem] pb-[4.2rem] portrait:flex-col portrait:py-[6.4rem]">
      ...
      <div className="flex flex-[6] flex-col justify-center">
        <div className="max-w-[90%]">
          ...
          <h4 className="mb-[2.4rem] text-[3.4rem] leading-[3.6rem] font-bold tracking-[0.04px] portrait:text-[3.6rem] portrait:leading-[3.8rem]">
            {title}
          </h4>
          ...
          <div className="flex items-center gap-[1.8rem]">
            {links?.map(({ href, title }) => {
              if (!href) return null
              return <CustomLink key={title} title={title} url={href} />
            })}
          </div>
        </div>
      </div>
